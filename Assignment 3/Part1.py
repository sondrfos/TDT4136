...
class Tile:		#tile class for every tile generated
	...
	def __init__(self, x, y, parent):		#initialize parent and coordinates
		self.xPos = x
		self.yPos = y
		self.parent = parent
...
class Board:
	...
	def genSucc(self, current):
		...
		for x_,y_ in array:					#iterate through all sucessor coordinates
			...
			#if inside board and not a wall append to successor list
			if 0 <= x_ < self.x and 0 <= y_ < self.y: 
				if self.board[y_][x_] == '#': continue
				succ.append(Tile(x_, y_, current))
		return succ
